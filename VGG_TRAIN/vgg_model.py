...
def build_vgg19(input, reuse=False):
    ...
    # create a dict to save the features from different layers
    net = {}
    # load the VGG weight from path in config.py
    vgg_rawnet = scipy.io.loadmat(config.vgg_model_path)
    vgg_layers = vgg_rawnet['layers'][0]

    # Centering the data - the mean of RGB is [123.6800, 116.7790, 103.9390]
